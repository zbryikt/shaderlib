...
var chokidar, http, fs, path, pug, stylus, markdown, jsYaml, uglify, lsc, useMarkdown, cwd, cwdRe, pugExtapi, pad, now, _log, ignoreList, ignoreFunc, typeTable, watchPath, mkdirRecurse, stylTree, ctype, ftype, sampleCgi, routeTable, server, log, filecache, updateFile, watcher, slice$ = [].slice, replace$ = ''.replace;
...
ignoreList = [/^server.ls$/, /^library.pug/, /^\.[^/]+/, /^node_modules\//, /^assets\//];
...
server = function(req, res){
  var filePath, relPath, dir, files, i$, len$, it, length, buf;
  ...
  if (useMarkdown) {
    if (/\.md$/.exec(filePath)) {
      ...
      buf = ['<meta charset="utf-8">', '<link rel="stylesheet" type="text/css" href="/assets/markdown-air/air.css"></link>', buf].join("");
      length = Buffer.byteLength(buf, 'utf-8');
    }
  }
  res.writeHead(200, {
    "Content-Length": length || buf.length,
...
updateFile = function(it){
  var src, ref$, type, cmd, des, files, result, file, e, srcs, i$, len$, code, desdir;
  ...
  if (type === 'ls') {
    if (/src\/ls/.exec(src)) {
      try {
        ...
        result = (function(){
          ...
          for (i$ = 0, len$ = (ref$ = files).length; i$ < len$; ++i$) {
            file = ref$[i$];
            results$.push(lsc.compile(fs.readFileSync(file).toString(), {
              bare: true
            }));
          }
          return results$;
        }()).join("");
        fs.writeFileSync("build.min.js", result);
      ...
      try {
        ...
        fs.writeFileSync(des, lsc.compile(fs.readFileSync(src).toString(), {
          bare: true
        }));
  ...
  if (type === 'styl') {
    ...
    for (i$ = 0, len$ = srcs.length; i$ < len$; ++i$) {
      ...
      try {
        ...
        stylus(code).set('filename', src).define('index', fn$).render(fn1$);
      } catch (e$) {
        e = e$;
        console.log("[BUILD]   " + src + " failed: ");
        console.log(e.message);
